...
int main(int argc, char** argv)
{
    ...
    IplImage *hsvimg, *hueimg, *satimg, *valimg;
    IplImage *filter_gray_img;
    ...
    /** Second command line argument is assumed to be the camera index
    ...
    */
    ...
    hueimg = cvCreateImage(cvGetSize(frame), frame->depth, 1);
    satimg = cvCreateImage(cvGetSize(frame), frame->depth, 1);
    valimg = cvCreateImage(cvGetSize(frame), frame->depth, 1);

    /// Create a single channel image to display filtered results
    filter_gray_img = cvCreateImage(cvGetSize(frame), frame->depth, 1);

    if( hsvimg == NULL || hueimg == NULL || satimg == NULL
       || valimg == NULL || filter_gray_img == NULL)
    {
        // Release the capture device housekeeping
        cvReleaseCapture( &capture );
        /** here it is possible that program is leaking memory in the
        *   case that one or more of the images were allocated */
        fprintf(stderr, "Not able to create all images\n");
        return -1;
    }
    ...
    cvNamedWindow("hue channel", CV_WINDOW_NORMAL | CV_WINDOW_KEEPRATIO);
    cvNamedWindow("val channel", CV_WINDOW_NORMAL | CV_WINDOW_KEEPRATIO);
    cvNamedWindow("sat channel", CV_WINDOW_NORMAL | CV_WINDOW_KEEPRATIO);
    cvNamedWindow("hsv filtered", CV_WINDOW_NORMAL | CV_WINDOW_KEEPRATIO);

    // display trackbar windows across top of screen
    cvMoveWindow("hue channel", 50, 50);
    cvMoveWindow("val channel", 425, 50);
    cvMoveWindow("sat channel", 800, 50);

    // display results across bottom of screen
    cvMoveWindow("camera", 50, 400);
    cvMoveWindow("hsv filtered", 425, 400);
    ...
    *
    */
    cvCreateTrackbar("h max", "hue channel", &hue_max, HUE_MAX, onHueMax);
    cvCreateTrackbar("h min", "hue channel", &hue_min, HUE_MAX, onHueMin);
    cvCreateTrackbar("s max", "sat channel", &sat_max, SAT_MAX, onSatMax);
    cvCreateTrackbar("s min", "sat channel", &sat_min, SAT_MAX, onSatMin);
    cvCreateTrackbar("v max", "val channel", &val_max, VAL_MAX, onValMax);
    cvCreateTrackbar("v min", "val channel", &val_min, VAL_MAX, onValMin);
    ...
    while( 1 )
    {
        ...
        */
        cvInRangeS(hsvimg, cvScalar(hue_min, sat_min, val_min, 0),
                    cvScalar(hue_max, sat_max, val_max, 0),filter_gray_img);

        /** Split into individual planes
        *
        *   void cvSplit(const CvArr* src, CvArr* dst0, CvArr* dst1, CvArr* dst2, CvArr* dst3)
        *   pass NULL for dst3 as we do not have a 4th channel in the hsvimg
        */
        cvSplit(hsvimg, hueimg, satimg, valimg, NULL);

        /** double cvThreshold(const CvArr* src, CvArr* dst, double threshold, double max_value,
                               int threshold_type)
        */
        /// Keep all values above hue_min
        cvThreshold(hueimg, hueimg, hue_min, hue_max, CV_THRESH_TOZERO);
        /// Keep all values below hue_max
        cvThreshold(hueimg, hueimg, hue_max, hue_max, CV_THRESH_TOZERO_INV);

        /// Keep all values above sat_min
        cvThreshold(satimg, satimg, sat_min, sat_max, CV_THRESH_TOZERO);
        /// Keep all values below sat_max
        cvThreshold(satimg, satimg, sat_max, sat_max, CV_THRESH_TOZERO_INV);

        /// Keep all values above val_min
        cvThreshold(valimg, valimg, val_min, val_max, CV_THRESH_TOZERO);
        /// Keep all values below val_max
        cvThreshold(valimg, valimg, val_max, val_max, CV_THRESH_TOZERO_INV);

        cvShowImage("camera", frame);
        cvShowImage("hue channel", hueimg);
        cvShowImage("sat channel", satimg);
        cvShowImage("val channel", valimg);
        cvShowImage("hsv filtered", filter_gray_img);
    ...
    cvReleaseImage(&filter_gray_img);
    cvDestroyAllWindows();
